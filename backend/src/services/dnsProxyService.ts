...
import { findMatchingDnsIndicator, getSecurityRelevantReasons } from './dnsIndicators';
import {
  AdGuardClient,
  AdGuardCredentials,
...
export function isCampaignRelevantDnsReason(reason?: string | null): boolean {
  ...
  return getSecurityRelevantReasons().includes(reason);
}
...
export function classifyDnsSignal(
  ...
  const indicator = findMatchingDnsIndicator({ domain, reason, rule });
  if (!indicator) return undefined;

  const description = (indicator.description || `${indicator.label} matched ${domain}.`).replace(
    /\{\{domain\}\}/g,
    domain
  );
...
export async function saveDnsProxySettings(input: DnsProxySettingsInput): Promise<DnsProxySettingsView> {
  ...
  const data = {
    ...
    allowSelfSigned: input.useHttps && input.allowSelfSigned === true,
...
export async function getDnsEvidenceForClients(params: {
  ...
  const queries = await prisma.dnsQueryEvent.findMany({
    where: {
      ...
      OR: [
        { isSuspicious: true },
        { reason: { in: getSecurityRelevantReasons() } },
      ],
    },
    orderBy: { queriedAt: 'desc' },
...
export async function getHourlyDnsBuckets(connectionId: string, since: Date): Promise<HourlyBucket[]> {
  const rows = await prisma.$queryRaw<Array<{ hour: Date; allowed: bigint; blocked: bigint }>>`
    SELECT date_trunc('hour', "queriedAt") AS hour,
           COUNT(*) FILTER (WHERE "isBlocked" = false) AS allowed,
           COUNT(*) FILTER (WHERE "isBlocked" = true) AS blocked
    FROM "DnsQueryEvent"
    WHERE "connectionId" = ${connectionId} AND "queriedAt" >= ${since}
    GROUP BY hour
    ORDER BY hour ASC
  `;
  return rows.map((r) => ({
...
export async function getTopDnsDomains(
  connectionId: string,
  since: Date,
  blocked: boolean,
  limit = 10
): Promise<TopDomainEntry[]> {
  const rows = await prisma.dnsQueryEvent.groupBy({
    by: ['domain'],
    where: { connectionId, queriedAt: { gte: since }, isBlocked: blocked },
    _count: { _all: true },
    orderBy: { _count: { domain: 'desc' } },
    take: limit,
  });
  return rows.map((r) => ({ domain: r.domain, count: r._count._all }));
}

export async function getClientAggregates(connectionId: string, since: Date): Promise<ClientAggregate[]> {
  ...
  >`
    SELECT "clientIp",
           ...
           COUNT(*) AS queries,
           COUNT(*) FILTER (WHERE "isBlocked" = true) AS blocked,
           COUNT(*) FILTER (WHERE "isSuspicious" = true) AS suspicious,
           MAX("queriedAt") AS "lastSeen"
    FROM "DnsQueryEvent"
